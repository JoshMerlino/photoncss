...
function Toolbar({ children, color, variant, size, position, id, className = "", ...props }) {
    ...
    setImmediate(function () {
        const toolbar = (0, jquery_1.default)(`#${id}`);
        toolbar.siblings(".photon-toolbar-spacer").remove();
        toolbar.before(`<div class="photon-toolbar-spacer" data-toolbar="${id}"></div>`);
        const spacer = (0, jquery_1.default)(`[data-toolbar="${id}"]`);
        spacer.height(toolbar.height() || 0);
        if (variant === "float") {
            (0, jquery_1.default)(document).on("scroll", () => {
                const scroll = (0, jquery_1.default)(document).scrollTop();
                if (scroll > 0) {
    ...
    const classes = (0, classnames_1.default)("photon-toolbar", `variant-${variant}`, `position-${position}`, `color-${color}`, `size-${size}`, className);
...
function ToolbarTitle({ children, subtitle, ...props }) {
    const classes = (0, classnames_1.default)("photon-toolbartitle", subtitle !== null && "contains-subtitle");
...
function ToolbarActions({ children, ...props }) {
    const classes = (0, classnames_1.default)("photon-toolbaractions");
