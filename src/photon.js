...
;(function(){
	...
	$.fn.stepper = function(){
		this.each(function(){
			var inc = 1;
			$(this).children(".stepper-items").children(".step").each(function(){
				$(this).prepend("<div class='number'>" + inc + "</div>");
				inc ++;
			}).click(function(){
				...
				$(".stepper-step").hide()
				$(".stepper-step[data-step='" + $(this).data("step") + "']").show();

				if($(this).is(":last-child")){
					$(this).parents(".stepper").children(".stepper-navigation").children(".btn").removeClass("disabled").last().addClass("disabled")
				} else if($(this).is(":first-child")){
					$(this).parents(".stepper").children(".stepper-navigation").children(".btn").removeClass("disabled").first().addClass("disabled")
				} else {
					$(this).parents(".stepper").children(".stepper-navigation").children(".btn").removeClass("disabled")
				}

				var p = $(this).parents(".stepper").children(".stepper-items").children().length - 1;
				var p = $(this).parents(".stepper").children(".stepper-items").children(".current").index()/p;

				$(this).parents(".stepper").children(".stepper-navigation").children(".progress").children().css("width",p*100 + "%")

				$(this).parent().children().removeClass("finished")
				$(this).prevAll().addClass("finished")

			}).first().click();
			$(this).children(".stepper-content").children().first().show();
			$(this).children(".stepper-navigation").children(".btn").click(function(){
				if($(this).is(":last-child")){
					$(this).parents(".stepper").children(".stepper-items").children(".current").removeClass("current").next().click();
				} else if($(this).is(":first-child")){
					$(this).parents(".stepper").children(".stepper-items").children(".current").removeClass("current").prev().click();
				}
			})
		})
	};
